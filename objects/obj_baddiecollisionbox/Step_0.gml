...
if (instance_exists(baddieID) && place_meeting(x, y, obj_player) && obj_player.cutscene == 0)
{
	if (baddieID.state != 101)
	{
		with (obj_player)
		{
			...
			if (instance_exists(other.baddieID) && state == 100 && sprite_index == spr_cotton_attack)
			{
				with (other.baddieID)
				{
					hp = 0;
			...
			if (instance_exists(other.baddieID) && !place_meeting(x, y, other.baddieID) && obj_player.state != 36 && obj_player.state != 90 && obj_player.state != 15 && other.baddieID.grounded == 1)
			{
				other.baddieID.state = 98;
