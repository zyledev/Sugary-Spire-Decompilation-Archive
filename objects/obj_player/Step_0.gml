...
with (obj_player)
{
	...
	if (!instance_exists(baddiegrabbedID) && (state = 39 || (state = 36 && sprite_index != spr_piledriverland) || state = 88))
		state = 0;
	if (!(state = 39 || state = 88 || state = 36 || state = 90))
		baddiegrabbedID = -4;
	if (character == "P")
	{
		if (anger = 0)
	...
	if (angry && !instance_exists(obj_angrycloud) && obj_player.state = 0)
...
if (global.starrmode)
{
	if (global.starrmode && state == 83) // I swear this second if statement is just sugary dev shenanigans
		movespeed = 12;
	else if (state = 83 && movespeed > 12)
		movespeed = 12;
}
if (state = 71 || place_meeting(x, y, obj_hallway) || state = 57)
	global.roomsave = false;
if (state == 90 && !floor(image_index) = (image_number - 1))
{
	with (instance_place(x, y, obj_baddie))
		vsp = 0;
	grav = 0;
}
if (state = 90 && floor(image_index) = (image_number - 0))
{
	with (instance_place(x, y, obj_baddie))
		vsp = 0.1;
	grav = 0.5;
